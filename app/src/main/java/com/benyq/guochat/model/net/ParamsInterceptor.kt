...
import com.benyq.guochat.app.FormUrlEncoded
...
object ParamsInterceptor : Interceptor {
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        if (canInjectIntoBody(request)) {
            ...
            formBodyBuilder.add("token", LocalStorage.token)
            formBodyBuilder.add("marketId", LocalStorage.uid)
            ...
            request = requestBuilder.url(request.url.newBuilder().run {
                addQueryParameter("token", LocalStorage.token)
                addQueryParameter("marketId", LocalStorage.uid)
            }.build()).build()
